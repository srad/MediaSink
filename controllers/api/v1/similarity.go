...
func SearchSimilarVideosByImage(c *gin.Context) {
	...
	ids := make([]database.RecordingID, 0, len(matches))
	for _, m := range matches {
		ids = append(ids, m.RecordingID)
	}
	recordings, err := database.FindRecordingsByIDs(ids)
	if err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
	recByID := make(map[database.RecordingID]*database.Recording, len(recordings))
...
func GroupSimilarVideos(c *gin.Context) {
	...
	ids := make([]database.RecordingID, 0, len(req.RecordingIDs))
	for _, id := range req.RecordingIDs {
		if id == 0 {
			continue
		}
		ids = append(ids, database.RecordingID(id))
	}
	...
	idSet := make(map[database.RecordingID]struct{})
	for _, g := range groups {
		for _, id := range g.RecordingIDs {
			idSet[id] = struct{}{}
		}
	}
	allIDs := make([]database.RecordingID, 0, len(idSet))
	for id := range idSet {
		allIDs = append(allIDs, id)
	}
	recs, err := database.FindRecordingsByIDs(allIDs)
	if err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
	recByID := make(map[database.RecordingID]*database.Recording, len(recs))
	...
	for i, g := range groups {
		videos := make([]*database.Recording, 0, len(g.RecordingIDs))
		for _, id := range g.RecordingIDs {
	...
	analyzedIDs, _ := database.ListRecordingIDsWithFrameVectors(1000)
