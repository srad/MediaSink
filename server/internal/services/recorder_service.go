...
import (
	...
	"github.com/srad/mediasink/server/internal/db" // Assuming db.Channel has ChannelID, ChannelName, IsPaused
...
func checkStreams(ctx context.Context) {
	initialChannelList, err := db.EnabledChannelList()
	if err != nil {
	...
	for _, initialChannelInfo := range initialChannelList {
		...
		wg.Add(1)
		go func(channelFromList *db.Channel) { // Pass initial channel info by value
			defer wg.Done()
			...
			currentChannelState, fetchErr := db.GetChannelByID(channelFromList.ChannelID)
			if fetchErr != nil {
			...
			if started && startErr != nil {
				...
				ws.BroadCastClients(ws.ChannelOfflineEvent, currentChannelState.ChannelID)
				...
				ws.BroadCastClients(ws.ChannelOnlineEvent, currentChannelState.ChannelID)
				ws.BroadCastClients(ws.ChannelStartEvent, currentChannelState.ChannelID)
			}
			if startErr != nil {
