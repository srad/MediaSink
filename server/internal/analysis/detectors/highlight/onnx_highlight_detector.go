...
func (d *onnxHighlightDetector) ExtractFeatures(frame image.Image) ([]float32, error) {
	...
	defer func() {
		if err := inputTensor.Destroy(); err != nil {
			log.Warnf("[onnxHighlightDetector] Failed to destroy input tensor: %s", err)
		}
	}()
	...
	defer func() {
		if err := outputs[0].Destroy(); err != nil {
			log.Warnf("[onnxHighlightDetector] Failed to destroy output tensor: %s", err)
		}
	}()
...
func (d *onnxHighlightDetector) DetectHighlights(frames []image.Image, timestamps []float64) ([]db.HighlightInfo, error) {
	if len(frames) < 2 {
		return nil, nil
	}
