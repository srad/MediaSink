...
type SimilarRecordingMatch struct {
	RecordingID   database.RecordingID
	Similarity    float64
...
type SimilarRecordingGroup struct {
	RecordingIDs  []database.RecordingID
...
func SearchSimilarRecordingsByImage(r io.Reader, minSimilarity float64, limit int) ([]SimilarRecordingMatch, error) {
	...
	results, err := database.SearchSimilarRecordingsByVector(vec, minSimilarity, limit)
	if err != nil {
		return nil, err
	}
...
func GroupSimilarRecordings(minSimilarity float64, recordingIDs []database.RecordingID, pairLimit int, includeSingletons bool) ([]SimilarRecordingGroup, error) {
	ids := recordingIDs
	if len(ids) == 0 {
		autoIDs, err := database.ListRecordingIDsWithFrameVectors(1000)
		if err != nil {
			return nil, err
		}
		ids = autoIDs
	...
	edges, err := database.QueryRecordingSimilarityEdges(minSimilarity, ids, pairLimit)
	if err != nil {
		return nil, err
	}

	type dsu struct {
		parent map[database.RecordingID]database.RecordingID
	}
	var find func(d *dsu, x database.RecordingID) database.RecordingID
	find = func(d *dsu, x database.RecordingID) database.RecordingID {
		p := d.parent[x]
		if p == x {
			return x
		}
	...
	union := func(d *dsu, a, b database.RecordingID) {
		ra := find(d, a)
		rb := find(d, b)
		if ra != rb {
			d.parent[rb] = ra
	...
	d := &dsu{parent: make(map[database.RecordingID]database.RecordingID, len(ids))}
	for _, id := range ids {
		d.parent[id] = id
	}
	for _, e := range edges {
	...
	componentIDs := make(map[database.RecordingID][]database.RecordingID)
	componentMax := make(map[database.RecordingID]float64)
	for _, id := range ids {
		root := find(d, id)
