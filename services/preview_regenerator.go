...
func RegenerateAllPreviews() error {
	...
	go func() {
		...
		if err != nil {
			...
			network.BroadCastClients(network.JobErrorEvent, fmt.Sprintf("Failed to fetch recordings: %v", err))
			return
		}
		...
		network.BroadCastClients(network.JobStartEvent, map[string]interface{}{
		...
		for i, rec := range recordings {
			...
			network.BroadCastClients(network.JobProgressEvent, map[string]interface{}{
				"type":         "preview_regeneration",
				"current":      current,
				"total":        total,
