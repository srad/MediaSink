...
func ExtractFirstFrame(input, outputPathPoster string) error {
	err := ExecSync(&ExecArgs{
		Command:     "ffmpeg",
		CommandArgs: []string{"-y", "-hide_banner", "-loglevel", "error", "-i", input, "-r", "1", "-vf", fmt.Sprintf("scale=-1:%d", config.FrameHeight), "-q:v", "2", "-frames:v", "1", outputPathPoster},
	})
...
func (video *Video) ExecPreviewFrames(args *VideoConversionArgs, videoDuration float64, previewFramesPath string) (*PreviewResult, error) {
	...
	err := ExecSync(&ExecArgs{
		...
		CommandArgs: []string{
			...
			"-y",
			"-progress", "pipe:1",
			"-threads", fmt.Sprint(config.ThreadCount),
			"-an",
...
//	dirPreview := filepath.Join(outputDir, config.ScreensFolder, filename)
...
//		},
//		Command:     "ffmpeg",
//		CommandArgs: []string{"-i", video.AbsoluteChannelFilepath, "-y", "-progress", "pipe:1", "-q:v", "0", "-threads", fmt.Sprint(config.ThreadCount), "-an", "-vf", fmt.Sprintf("select=not(mod(n\\,%d)),scale=-2:%d", frameDistance, frameHeight), "-hide_banner", "-loglevel", "error", "-stats", "-fps_mode", "vfr", filepath.Join(dirPreview, outFile)},
//	})
//}
