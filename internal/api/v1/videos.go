...
import (
	...
	"github.com/srad/mediasink/internal/models"
...
// @Accept      json
// @Produce     json
// @Success     200 {object} []db.Recording
// @Failure     500 {} string "Error message"
// @Router      /videos [get]
func GetVideos(c *gin.Context) {
	appG := app.Gin{C: c}
	videos, err := db.RecordingsList()
...
func FavVideo(c *gin.Context) {
	...
	if err := db.FavRecording(uint(id), true); err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
...
func UnfavVideo(c *gin.Context) {
	...
	if err := db.FavRecording(uint(id), false); err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
...
func FilterVideos(c *gin.Context) {
	...
	if !request.SortOrder.IsValid() {
		request.SortOrder = models.SortDesc
	}
	...
	videos, totalCount, err := db.SortBy(request.SortColumn.String(), request.SortOrder.String(), request.Skip, request.Take)
	if err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
...
func GetRandomVideos(c *gin.Context) {
	...
	videos, err := db.FindRandom(limit)
...
func MergeVideos(c *gin.Context) {
	...
	job, err := db.EnqueueMergeJob(db.ChannelID(channelID), mergeRequest.RecordingIDs, mergeRequest.ReEncode)
	if err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
...
func EnhanceVideo(c *gin.Context) {
	...
	targetRes := util.ResolutionType(enhanceRequest.TargetResolution)
	encodingPreset := util.EncodingPreset(enhanceRequest.EncodingPreset)
	...
	job, err := db.EnqueueEnhanceVideoJob(uint(id), enhanceArgs)
	if err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
...
func EstimateEnhancement(c *gin.Context) {
	...
	// Get recording
	recording, err := db.RecordingID(id).FindRecordingByID()
	if err != nil {
		appG.Error(http.StatusInternalServerError, err)
		return
	}
	...
	// Convert to helpers types
	targetRes := util.ResolutionType(estimateRequest.TargetResolution)
