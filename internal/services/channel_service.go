...
func CreateChannel(name, displayName string, skipStart, minDuration uint, url string, fav bool, tags *db.Tags, isPaused bool) (*ChannelInfo, error) {
	channel := db.Channel{
	...
	newChannel, err := db.CreateChannelDetail(channel)
...
func GetChannels() ([]ChannelInfo, error) {
	channels, err := db.ChannelListNotDeleted()
	if err != nil {
		return nil, err
	}
...
func GetChannel(id uint) (*ChannelInfo, error) {
	channelID := db.ChannelID(id)
	channel, err := db.GetChannelByIDWithRecordings(channelID)
	if err != nil {
...
func UploadRecording(channelID db.ChannelID, fileReader io.Reader) (*db.Recording, error) {
	// Create recording entry and get output path
	recording, outputPath, err := db.NewRecording(channelID, "recording")
	if err != nil {
	...
	videoInfo := &util.Video{FilePath: outputPath}
