...
var (
	...
	recInfo    = make(map[db.ChannelID]*db.Recording)
	streamInfo = make(map[db.ChannelID]*StreamInfo) // Changed to pointers to avoid lost updates on concurrent modifications
	streams    = make(map[db.ChannelID]*exec.Cmd)
...
func (si *StreamInfo) Screenshot() error {
	// Using absolute path for ffmpeg in util.ExtractFirstFrame.
	return util.ExtractFirstFrame(si.URL, filepath.Join(si.ChannelName.AbsoluteChannelDataPath(), db.SnapshotFilename))
}
...
func CaptureChannel(id db.ChannelID, url string, skip uint) error {
	...
	recording, outputFilePath, err := db.NewRecording(channel.ChannelID, "recording")
	if err != nil {
		activeRecLock.Unlock() // Unlock before returning error
	...
	currentChannelState, errChannel := db.GetChannelByID(id) // Re-fetch for latest MinDuration
	if errChannel == nil {
	...
	if recDuration.Minutes() >= channelDuration {
		...
		if ok && recToFinalize.Filename == recording.Filename {
			info := Info(id)
			if newRecording, err := db.CreateRecording(info.ChannelID, info.Filename, "recording"); err != nil {
				...
				ws.BroadCastClients(ws.RecordingAddEvent, newRecording)
...
func GetRecordingMinutes(id db.ChannelID) float64 {
	activeRecLock.Lock()
...
func Info(id db.ChannelID) *db.Recording {
	activeRecLock.Lock()
	defer activeRecLock.Unlock()
...
func Start(id db.ChannelID) (bool, error) {
	channel, err := db.GetChannelByID(id)
	if err != nil {
...
func TerminateAll() {
	...
	idsToTerminate := make([]db.ChannelID, 0, len(streams))
...
func TerminateProcess(id db.ChannelID) error {
	activeRecLock.Lock()
	...
	var channelNameForLog db.ChannelName
...
func IsOnline(id db.ChannelID) bool {
	streamInfoLock.Lock()
...
func IsTerminating(id db.ChannelID) bool {
	streamInfoLock.Lock()
	defer streamInfoLock.Unlock()
	si, ok := streamInfo[id]
...
func IsRecordingStream(id db.ChannelID) bool {
	activeRecLock.Lock()
	defer activeRecLock.Unlock()
	_, ok := streams[id]
	return ok
...
func DeleteStreamData(id db.ChannelID) {
...
func startThumbnailWorker(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			infosToSnapshot := make(map[db.ChannelID]StreamInfo)
			...
			for channelID, infoCopy := range infosToSnapshot { // Iterate over the copied structs
				...
				if err := infoCopy.Screenshot(); err != nil {
					...
					ws.BroadCastClients(ws.ChannelThumbnailEvent, channelID) // Broadcast original channelID
				}
			}
		}
	}
