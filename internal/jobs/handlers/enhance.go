...
func (h *enhanceHandler) Handle(job *db.Job, threadCount int) error {
	enhanceArgs, err := db.UnmarshalJobArg[util.EnhanceArgs](job)
	if err != nil {
		return err
	}
	...
	inputVideo := &util.Video{FilePath: inputFile}
	...
	filename := db.RecordingFileName(fmt.Sprintf("%s_enhanced_%s_%s.mp4", job.ChannelName, enhanceArgs.TargetResolution, stamp))
	...
	err = util.ExecSync(&util.ExecArgs{
		...
		OnPipeErr: func(info util.PipeMessage) {
			...
			if !strings.Contains(info.Output, "x265 [info]") &&
				...
				ws.BroadCastClients(ws.JobErrorEvent, JobMessage[string]{Job: job, Data: info.Output})
			}
		},
	})
	...
	enhancedRecording, errCreate := db.CreateRecording(job.ChannelID, filename, "enhanced")
	if errCreate != nil {
		if errCleanup := os.Remove(outputFile); errCleanup != nil {
