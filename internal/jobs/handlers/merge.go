...
func (h *mergeHandler) Handle(job *db.Job, threadCount int) error {
	...
	for i, recID := range recordingIDs {
		rec, err := db.RecordingID(recID).FindRecordingByID()
		if err != nil {
	...
	filename := db.RecordingFileName(fmt.Sprintf("%s_merged_%s.mp4", job.ChannelName, stamp))
	...
	if reEncode {
		// Re-encoded merge
		errMerge = util.MergeVideosReEncoded(&util.MergeReEncodeArgs{
			...
			OnProgress: func(info util.PipeMessage) {
				ws.BroadCastClients(ws.JobProgressEvent, JobMessage[string]{Job: job, Data: info.Output})
			},
		...
		concatFilePath := job.ChannelName.AbsoluteChannelFilePath(db.RecordingFileName(fmt.Sprintf("merge_concat_%d.txt", now.UnixNano())))
	...
	mergedRecording, errCreate := db.CreateRecording(job.ChannelID, filename, "merged")
	if errCreate != nil {
		if errCleanup := os.Remove(outputFile); errCleanup != nil {
