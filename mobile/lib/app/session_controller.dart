...
typedef AppSessionApiFactory = MediaSinkApi Function({
  required ServerConfig config,
  String? token,
  UnauthorizedHandler? onUnauthorized,
});
typedef AppSessionSocketFactory = MediaSinkSocketService Function({
  required ServerConfig config,
...
class AppSessionController extends ChangeNotifier {
  AppSessionController({
    AppSessionStorage? storage,
    AppBuildInfoLoader? buildInfoLoader,
    AppSessionApiFactory? apiFactory,
    AppSessionSocketFactory? socketFactory,
    bool autoBootstrap = true,
  }) : _storage = storage ?? const SecureAppSessionStorage(),
       _buildInfoLoader = buildInfoLoader ?? MediaSinkApi.fetchBuildInfo,
  ...
  static MediaSinkApi _defaultApiFactory({
    required ServerConfig config,
    String? token,
    UnauthorizedHandler? onUnauthorized,
  }) {
  ...
  static MediaSinkSocketService _defaultSocketFactory({
    required ServerConfig config,
    required String token,
  }) {
  ...
  Future<void> bootstrap() async {
    ...
    _savedOrigin = await _storage.read(key: _serverOriginKey);
    _savedUsername = await _storage.read(key: _usernameKey);
  ...
  Future<void> configureServer(String origin) async {
    ...
    try {
      ...
      _config = config;
      await _storage.write(key: _serverOriginKey, value: normalized);
  ...
  Future<void> resetServer() async {
    await logout();
    await _storage.delete(key: _serverOriginKey);
    _config = null;
    _savedOrigin = null;
    _setState(SessionStatus.unconfigured, message: null);
  }
  ...
  Future<void> _migrateLegacyStorage() async {
    final legacyServerUrl = await _storage.read(key: _legacyServerUrlKey);
    final legacyUsername = await _storage.read(key: _legacyUsernameKey);
    final legacyToken = await _storage.read(key: _legacyJwtKey);

    if ((await _storage.read(key: _serverOriginKey)) == null && legacyServerUrl != null && legacyServerUrl.isNotEmpty) {
      await _storage.write(key: _serverOriginKey, value: ServerConfig.normalizeOrigin(legacyServerUrl));
    }
    if ((await _storage.read(key: _usernameKey)) == null && legacyUsername != null && legacyUsername.isNotEmpty) {
      await _storage.write(key: _usernameKey, value: legacyUsername);
    }
    if ((await _storage.read(key: _tokenKey)) == null && legacyToken != null && legacyToken.isNotEmpty) {
      await _storage.write(key: _tokenKey, value: legacyToken);
    }

    await _storage.delete(key: _legacyWebSocketUrlKey);
